<template>
  <v-container fluid class="mx-0 pa-0" :style="styleBlur">
    ...
    <v-container
      fluid
      style="position:absolute;top:50%;left:55%;z-index:90"
      v-if="checkIfGlassesOnScreen()"
      @click="getGlasses()"
    >
      <v-img
        :src="this.$store.getters.getImageById('oculos')"
        max-height="100"
        max-width="100"
      ></v-img>
    </v-container>
        ...
        >
          <v-col cols="12" sm="4">
            <v-card
              ...
              class="caixaTexto"
              tile
              @click="chooseOption(escolha.index)"
            >
              <v-card-text>
                {{ escolha.msg }}
              </v-card-text>
    ...
    <v-dialog v-model="phone" fullscreen>
